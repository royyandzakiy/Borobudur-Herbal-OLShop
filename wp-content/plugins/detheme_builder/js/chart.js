        ...
        $j('.doughnutChart').each(function () {
            $j(this).appear(function () {
                ...
                var sclr=$piechart.data('active'),activeStrokeColor="#19bd9b",nsclr=$piechart.data('noactive'),nonactiveStrokeColor="transparent",
                    strokewidth=$piechart.data('size'),counterunit=$piechart.data('unit'),fontRatio=$piechart.data('font-ratio'),FontSize;

                if(!fontRatio){
                    fontRatio=0.16;
                }

                fontRatio=Math.max(0.16,fontRatio);
                fontRatio=Math.min(1,fontRatio);

                $tocounter.css({"line-height":$piechart.get(0).height+'px','font-size':($piechart.get(0).height * fontRatio)+'px'});
                $wrapper.css("height",$containerHeight+"px");

                if(counterunit){
                    FontSize=$tocounter.css('font-size');
                    FontSize= parseFloat(FontSize) *0.6;
                    $tocounterunit.html(counterunit);
                    $tocounterunit.css({'font-size':FontSize+'px','top':'-'+(FontSize*0.4)+'px'});
                }
...
//                    initToCounterHorizontalProgressBar();
                    ...
                    if ($animation) {
                        ...
                        $j(this).find('.progress_content').css({'width':percentage + '%','background-color': color});
                        $j(this).find('.progress_number').css('opacity', '1');
                    }
