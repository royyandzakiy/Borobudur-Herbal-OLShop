...
/**
*
* fix for server not load filter_var module
*
*/
...
if(!function_exists('get_detheme_number')){
...
// Don't duplicate me!
if( !class_exists( 'DethemeReduxFramework' ) ) {
...
**/
         ...
         */
        public function __render( $sections = array(), $args = array(), $extra_tabs = array() ) {
            global $wp_version;
        ...
        function get_options() {
            ...
            if ( empty( $result ) && $defaults = $this->_default_values() ) {
...
/**
        ...
        function _options_page() {
            ...
            if( $this->args['page_type'] == 'submenu' ) {
                ...
                add_theme_page($this->args['page_title'], $this->args['menu_title'], $this->args['page_cap'], $this->args['page_slug'], array( &$this, '_options_page_html' ));
            ...
            add_action( 'admin_print_styles',create_function('','if(is_rtl()) wp_enqueue_style( \'redux-rtl-css\' );'));
                ...
                if( isset( $section['fields'] ) ) {
                    ...
                    foreach( $section['fields'] as $fieldk => $field ) {
                        if( isset( $field['type'] ) && $field['type'] != "callback"  ) {
                            $field_class = 'DethemeReduxFramework_' . $field['type'];
...
//                                $class_file = apply_filters( 'redux/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
                                $class_file = apply_filters( 'redux/'.$this->args['opt_name'].'/field/class/'.$field['type'], 'redux-framework/ReduxCore/inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field );
        ...
        public function _enqueue() {
           ...
           wp_register_style(
                'redux-rtl-css',
                self::$_url . 'assets/css/redux-rtl.css',
                '',
                filemtime( self::$_dir . 'assets/css/redux-rtl.css' ),
                'all'
            );
        ...
        public function _register_setting() {
            ...
            $theme=get_template();
            if ( isset( $_GET[sanitize_key( 'detheme_notification_'.$theme)] ) ){
                update_user_meta( get_current_user_id(), 'hide_'.$theme.'_detheme_notification', 1 );
            }
            if(($pagenow=='themes.php'  && $this->args['page_slug']==(isset($_GET['page'])?$_GET['page']:"") ) || isset($_POST[$this->args['opt_name']])){
            
            }
            else{

               return $this->_get_theme_info(get_option('detheme_license_'.get_template()));
            }
            ...
            if ( $runUpdate || get_transient( 'redux-compiler-' . $this->args['opt_name'] ) ) {
        ...
        public function _register_extensions() {     
            ...
            $path = 'redux-framework/ReduxCore/extensions/';
            ...
            foreach($folders as $folder){
                ...
                $extension_class = 'DethemeReduxFramework_Extension_' . $folder;
                    ...
                    foreach( $section['fields'] as $field ) {
                        ...
                        if( isset( $field['validate'] ) ) {
                            ...
                            if( !class_exists( $validate ) ) {
                                $class_file = apply_filters( 'redux-validateclass-load', 'redux-framework/ReduxCore/inc/validation/' . $field['validate'] . '/validation_' . $field['validate'] . '.php', $validate ); // REMOVE LATER
                        ...
                        elseif(isset( $field['validate_callback'] ) && $field['id']=='detheme_license' && !function_exists( $field['validate_callback'] )){
                            $plugin_options[$field['id']] = "";

                            $field['msg']=__('L'. 'i' .'c'. 'e'. 'n'. 's'. 'e'. ' '.'n'. 'u'. 'm'.'b'.'e'.'r'.' n'.'o'.'t'.' '.'v'.'a'.'l'.'i'.'d'.','.' '.'p'.'l'.'e'.'a'.'s'.'e'.' '.'t'.'r'.'y'.' '.'a'.'g'.'a'.'i'.'n'.'.','billio');    
                            $this->errors[] = $field;                     
                        }
        ...
        public function _options_page_html() {
            ...
            $this->_get_detheme_update(get_option("detheme_license_".get_template()));
            ...
            if( $this->args['show_import_export'] === true ) {
                ...
                echo '</textarea>';
                /** @noinspection PhpUndefinedConstantInspection */
                echo '<input type="text" class="large-text noUpdate" id="redux-export-link-value" value="' . esc_url(add_query_arg( array( 'feed' => 'reduxopts-' . $this->args['opt_name'], 'secret' => md5( AUTH_KEY.SECURE_AUTH_KEY ) ), site_url() )) . '" />';
        ...
        private function _get_detheme_update($theme=""){
           return $this->_get_theme_info($theme);
        }
        ...
        public function check_dependencies($field) {
            ...
            if (!empty($field['required'])) {
                $fielddata['check-field'] = $field['required'][0];
                $fielddata['check-comparison'] = $field['required'][1];
                $fielddata['check-value'] = $field['required'][2];
                $params['data_string'] = $this->create_data_string($fielddata);
                $return = false;
                //required field must not be hidden. otherwise hide this one by default
                if (!isset($this->fieldsHidden[$fielddata['check-field']])) {
                    if (isset($this->fieldsValues[$fielddata['check-field']])) {
                        $value1 = $this->fieldsValues[$fielddata['check-field']];
                        $value2 = $fielddata['check-value'];
                        switch ($fielddata['check-comparison']) {
                            case '=': 
                            case 'equals': 
                                if(is_array($value2)){
        ...
        function _get_theme_info($id=""){
            global $pagenow;
            $theme=get_template();

            if($id==''){
                if(!get_user_meta( get_current_user_id(), 'hide_'.$theme.'_detheme_notification', 1 ))
                    return add_action( 'admin_notices', array( $this, 'install_notice' ) );
            }
         ...
         *
         * @param array $sourcedata example input: array('id'=>'true')
         * @return string $data_string example output: data-id='true'
         */
        public function create_data_string($sourcedata = array()){
            $data_string = "";
...
jQuery(document).ready(function($){
    ...
    $('.register-button').click(function(e){
       ...
       var  expires = "; expires=" + date.toGMTString();
        document.cookie = escape('redux_current_tab_get') + "=" + escape('notinfo') + expires + "; path=/";

    });
});
</script>
<div id="message" class="updated">
    <p><strong><?php printf(__( 'Your website using %s template', 'billio' ),$item_name); ?></strong></p>
    <p><?php _e('Please enter your Item Purchase Code to get theme & plugin update notification','billio');?></p>
    <p class="submit">
     <a href="<?php echo admin_url('themes.php?page='.$this->args['page_slug']); ?>&tab=info" class="register-button"><?php _e( 'Register Template', 'billio' ); ?></a> | 
     <a href="<?php print esc_url(add_query_arg( 'detheme_notification_'.strtolower($item_name), 'hide_'.strtolower($item_name).'_detheme_notification' )); ?>" target="_parent"><?php _e( 'Hide this notice', 'billio' );?></a>
</a>
    </p>
</div>
        <?php
    ...
    do_action( 'redux/init', DethemeReduxFramework::init() );
