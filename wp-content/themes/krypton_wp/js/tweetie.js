    ...
    $.fn.twittie = function () {
        ...
        $.getJSON(settings.apiPath, { username: settings.username, count: settings.count, exclude_replies: settings.hideReplies }, function (twt) {
            that.find('span').fadeOut('fast', function () {
                that.html('<ul class="sequence-canvas"></ul>');
                ...
                for (var i = 0; i < settings.count; i++) {
                    if (twt[i]) {
                        ...
                        var animatein = '';
                        if (i==0) animatein = ' class="animate-in" ';
                        that.find('ul').append('<li '+animatein+'><p class="slide-tweet-text">' + templating(temp_data) + '</p></li>');
                    } else {
                        break;
                    }
                }
